...
from visualize_utils import pcl_features_to_RGB
...
class BEV_corr(nn.Module):
    ...
    def forward(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        grd_feat_proj,mask = self.grd_f2BEV(grd_feat, uv)
        mask = mask.detach()
        # pcl_features_to_RGB([nn.Upsample(scale_factor=8, mode='nearest')(grd_feat)], 0, "result_visualize/PCA/grd_frat.jpg")
        # pcl_features_to_RGB([nn.Upsample(scale_factor=8, mode='nearest')(grd_feat_proj*mask)], 0, "result_visualize/PCA/BEV_mask.jpg")
        # pcl_features_to_RGB([nn.Upsample(scale_factor=8, mode='nearest')(grd_feat_proj)], 0, "result_visualize/PCA/BEV_nomask.jpg")
