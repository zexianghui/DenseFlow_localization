...
def match(mask, rot, tran_x, tran_y, coords0):
    ...
    for i in range(B):
        ...
        points_tran = (tra.float()@(torch.inverse(rol_center)).float()@rol_tra.float()@rol_center.float()@coords1_i)
...
def train(net, lr, args, save_path):
    ...
    for epoch in range(args.resume, args.epochs):
        ...
        if args.dpp:
            trainloader,train_sampler = load_vigor_data(args.batch_size, area=args.area, rotation_range=args.rotation_range,
                                                 train=True, weak_supervise=True,ddp = 1)
            train_sampler.set_epoch(epoch)
        else:
             trainloader, valloader = load_vigor_data(args.batch_size, area=args.area, rotation_range=args.rotation_range,
                                                 train=True, weak_supervise=True,ddp = 0)
        ...
        for Loop, Data in enumerate(trainloader, 0):
            ...
            optimizer.zero_grad()
            if args.dpp:
                grd, sat, gt_shift_u, gt_shift_v, gt_heading, meter_per_pixel = [item.cuda(args.local_rank) for item in Data]
            else:
            ...
            if args.end2end == 0:
                flow_predictions, flow_conf, mask = net(sat, grd, meter_per_pixel, \
                                                        ...
                                                        torch.zeros_like(gt_heading,device=gt_heading.device), mode='test', \
                                                        file_name=0)
...
def parse_args():
    ...
    parser.add_argument('--num_steps', type=int, default=40*10000)
