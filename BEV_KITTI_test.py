...
from visualize_utils import line_point, save_img, cut_cat, RGB_KITTI_com_pose
...
def test1(net_test, args, save_path, best_rank_result, epoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        # for h in range(coords0_gt.size()[0]):
        #     for w in range(coords1_gt.size()[1]):
        #         if ((h == 350 and w == 340) or \
        #             (random.randint(0,500) == 1)) and mask[0,0,h,w]:
        #             if coords1_gt[h][w][0].data.float().cpu()<512 and coords1_gt[h][w][0].data.float().cpu()>0 \
        #                 and coords1_gt[h][w][1].data.float().cpu()<512 and coords1_gt[h][w][1].data.float().cpu()>0:
        #                 x = [coords0_gt[h][w][0].data.float().cpu(), coords1_gt[h][w][0].data.float().cpu() + coords1_gt.size()[1]]
        #                 y = [coords0_gt[h][w][1].data.float().cpu(), coords1_gt[h][w][1].data.float().cpu()]

        #                 # x = [coords0_gt[h][w][0].data.float().cpu(), coords1_gt[h][w][0].data.float().cpu()]
        #                 # y = [coords0_gt[h][w][1].data.float().cpu(), coords1_gt[h][w][1].data.float().cpu()+ coords1_gt.size()[1] +20]
        ...
        # line_point('result_visualize/left_half.jpg', 'result_visualize/right_half.jpg', match_x, match_y, None, 'line_BEV_sat.jpg')
        ...
        #     com_u = mcc_v/ utils.get_meter_per_pixel(scale=1), com_v = mcc_u/ utils.get_meter_per_pixel(scale=1),come_theta=mcc_thate,\
